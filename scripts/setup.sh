...
if [ ! -f $GH_SETUP ]; then
    ...
    if [ -z "$GITHUB_USER_LOGIN" ]; then
        if [[ "$PRE_PUSH_HOOK_AUTH" == "PRE_PUSH_HOOK_TOKEN" ]]; then
            GITHUB_USER_LOGIN=$(git gui--askpass "Please enter your pre-push hook token:")
        else
            echo "Please enter your Git username from your primary git provider: "
